...
export  async function getETHBalance(address: string): Promise<ProviderSource> {
  try {
    ...
    return {hasError: data.status === "0", source: Providers.etherscan, value: formatNumber(data.result), time }
  } catch (error) {
    return errorResult(error, Providers.etherscan)
  }
}
...
export async function getDaiBalance(address: string): Promise<ProviderSource> {
  try {
    const response = await fetch(`https://api.etherscan.io/api?module=account&action=tokenbalance&contractaddress=${DAI_TOKEN_ADDRESS}&address=${address}&tag=latest&apikey=${API_KEY}`)
    const time = Date.now()
