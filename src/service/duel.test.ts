...
describe(`duel()`, () => {
  ...
  describe("when both parties return but results differ", () => {
    ...
    it("most recent wins", async () => {
      ...
      expect(global.console.info).toHaveBeenCalledWith("Sources: etherscan (100) differs from coinbase (102) 1.9608%")
      expect(result).toEqual(
        {
          "sources": ["coinbase"],
          "time": 1614121843684,
  ...
  describe("with errors", () => {
    ...
    describe("when neither party responds", () => {
      it("returns invalid", async () => {
        ...
        expect(global.console.warn).toHaveBeenCalledWith("Error ecb.europa.eu & ecb.europa.eu could not get new data")
