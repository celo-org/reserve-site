...
export default async function duel(alef:ProviderPromise, bet: ProviderPromise): Promise<Duel> {
  ...
  if (sourceA.value === sourceB.value) {
    return {value: sourceA.value, time: Math.max(sourceA.time, sourceB.time), sources: results.map(result => result.source)}
  }

  if (sourceA.value !== sourceB.value) {
    const recent = sourceA.time > sourceB.time ? sourceA : sourceB
    console.info(`Sources: ${sourceA.source} (${sourceA.value}) differs from ${sourceB.source} (${sourceB.value}) ${percentDif(sourceA.value, sourceB.value)}%` )
